...
// Honors direct UID fields on the request — useful for tests / internal callers
// that pass UIDs directly to bypass the resolver.
func (s *RunsService) resolveComponentScope(
	ctx context.Context,
	scope *types.ComponentSearchScope,
) (*internalSearchScope, error) {
	if scope.ComponentUID != "" && scope.EnvironmentUID != "" {
	...
	// Fall back to name-based resolution via the shared resolver helper.
